...
Production version for organizing after-school programs
...
```bash
mkvirtualenv iktel_production -p python3.12
...
Create a database named `iktel_production`.

```
createdb iktel_production
```
...
```bash
celery -A iktel_production worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A iktel_production worker -l INFO -B --pool=solo
```
